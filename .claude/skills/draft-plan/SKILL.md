...
- **공통 스키마·네이밍 규약을 문서화하기 전에 실제 소스 코드를 Read해서 확인한다** — 기억이나 가정으로 taxonomy/registry 문서를 쓰면 실제 코드와 어긋난 채로 여러 후속 산출물에 퍼진다.
- **여러 Task에 걸친 단일 데이터 파일은 한 Task에서 한 번에 저술한다** — 조각내면 리뷰 시점마다 "이 파일이 왜 또 바뀌나"를 다시 파악해야 한다. 후속 Task는 그 데이터를 소비하는 로직/UI만 추가한다.
- **여러 비정형 원본에서 "정본 레지스트리"를 만들어야 할 때는 손으로 전사한 별도 레지스트리보다, 실 데이터가 쌓이는 대로 점진적으로 실행되는 검증 로직(예: 충돌 검사 테스트)을 우선한다** — 손 전사는 이중 출처가 되어 오타 시 서로 어긋난다.
