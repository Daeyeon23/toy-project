...
applied: discarded
---
## E2E 불변 규칙 체크가 도메인 콘텐츠의 정당한 "%"까지 걸러낸 것은 테스트 설계 결함

**상황**: Task 11에서 크루아상 E2E를 추가하니 `bodyText.not.toMatch(/%/)`가 실패했다 — `lam-low-fat-butter` 원인의 조언 문구("지방 82% 이상의 유럽식 버터")에 정당한 "%"가 있었다. grep해 보니 white-loaf.ts(v1 원본 데이터)에도 이미 "5~10%"가 있었지만, v1 E2E는 그 원인이 안 뜨는 증상 조합만 테스트해서 우연히 안 걸렸을 뿐이다.
**판단**: 본문 전체를 "%" 정규식으로 스캔하는 대신, "가능성 높음/중간" 강도 라벨이 보이는지로 좁혔다. 원시 점수 비노출이라는 실제 보장은 `CauseResult` 타입에 숫자 필드가 없다는 걸 확인하는 `scoring.test.ts`가 이미 구조적으로 검증하고 있어, E2E는 그 위에 중복 검증할 필요가 없었다. 콘텐츠(조리법 설명)와 진단 메커니즘(점수)을 같은 정규식으로 묶어 검사한 것 자체가 처음부터 취약한 설계였다.
**다시 마주칠 가능성**: 낮음 — 이번 발견으로 이 저장소의 두 E2E 테스트 모두 올바른 형태로 고정됐고, 향후 원인 문구에 "%"가 더 들어가도 재발하지 않는다. 다른 프로젝트에서 "본문 전체 텍스트 정규식 스캔"으로 불변 규칙을 검사할 때는 도메인 콘텐츠가 그 정규식에 우연히 걸릴 수 있다는 점을 미리 고려할 만하다.

---
category: code-review
applied: not-yet
---
## code-reviewer의 "테스트 flaky는 이 PR의 회귀" 판정을 절반만 신뢰할 수 있는 A/B로 내렸다

**상황**: v3(제과류 파일럿) Task 12~16 리뷰에서 code-reviewer가 `bun run test` 타임아웃 flaky를 "이 PR이 일으킨 회귀"로 보고했다. 근거로 든 A/B는 "HEAD에서 전체 스위트 2회 실행 → 4·2건 실패" vs "base 커밋에서 **해당 파일만 격리 실행** 3회 → 항상 안정". 이 두 조건은 실행 모드(전체 스위트 vs 단일 파일)가 다르므로 인과 비교가 아니다.
**판단**: 직접 `git worktree`로 진짜 이전 커밋(pastry 작업 시작 전, f4c4992)을 체크아웃해 **똑같이 전체 스위트**를 돌려봤더니 같은 파일(`bread-doctor.test.tsx`)에서 동일한 타임아웃이 재현됐다 — 이 저장소의 4코어 환경에서 전체 스위트 병렬 실행 시 이미 있던 리소스 경합성 flaky였음을 확인. 리뷰어의 "회귀" 프레이밍은 기각하되, 제안한 메모이제이션(`useMemo`)은 그 자체로 타당한 개선이라 별도로 반영했다. commit 메시지에 판단 근거를 남겼다.
**다시 마주칠 가능성**: 높음 — code-reviewer/다른 에이전트가 "테스트가 불안정해졌다"고 보고할 때, 비교 대상의 **실행 모드(전체 스위트 vs 단일 파일, 병렬 vs 순차)를 동일하게 맞춘 진짜 base 커밋 A/B**가 아니면 회귀 여부를 판단할 수 없다는 패턴은 다른 feature에서도 반복될 만하다. `.claude/rules/`에 "flaky 회귀 판정 전 동일 실행 모드로 base 커밋 재현" 규칙으로 승격할지 사용자에게 확인이 필요하다.

---
category: task-ordering
applied: not-yet
---
## 기존 KB가 이미 품목-불가지론적(item-agnostic) DI 구조라면, 새 도메인 추가에 "배선 재구조화" Task가 필요 없다

**상황**: v2(25종 빵 확장)는 Task 2·3에서 KB를 전역 → 빵ID별로 재구조화하고 훅에 배선하는 별도 Task가 필요했다. v3(제과류 8종) draft-plan 때, `useDiagnosis`/`getBreadKnowledge`/`BreadType`이 이미 문자열 id 하나로 완전히 일반화돼 있어 "빵"이든 "제과류"든 구분하지 않는다는 걸 코드를 직접 읽고 확인했다.
**판단**: v2와 똑같은 순서(레지스트리 → 배선 재구조화 → 데이터)를 기계적으로 반복하지 않고, "매트릭스 작성 → UI 카피 일반화 → 데이터 인코딩 → 통합 검증"으로 plan을 다시 설계했다. 실제로 `hooks/use-diagnosis.ts`·`lib/bread-doctor/scoring.ts`·`search.ts`는 이번 v3 전체에서 단 한 줄도 안 바뀌었다 — 아키텍처 재사용이 그대로 증명됐다.
**다시 마주칠 가능성**: 중간 — "이미 확장 가능하게 설계된 시스템에 같은 종류의 항목을 늘릴 때"는 이전 확장의 Task 구조를 그대로 베끼기 전에, 실제 코드가 여전히 새 축을 구분 없이 받는지부터 확인하는 습관이 일반화할 만한 패턴이다.

---
category: refactor
applied: not-yet
---
## 매트릭스 8개를 빠르게 설계하려면 "품목당 1개 이상 원인-고유(signature) 증상"을 먼저 고정한다

**상황**: Task 12에서 8개 신규 카테고리(휘낭시에·브라우니·타르트·마카롱·붓세·시폰케이크·머핀(퀵브레드)·슈)의 증상×원인 매트릭스를 매번 손으로 가중치를 조정하며 PEAKED를 확인하다 보니 되풀이 계산이 많았다.
**판단**: 각 품목마다 "가중치 2점이며 다른 원인은 전혀 안 건드리는 증상"을 하나 이상 먼저 설계해두면(예: 마카롱의 `mac-sticky-undried`는 오직 스킨 부족에서만 나옴), 그 증상 하나만으로도 단독 PEAKED가 보장되고 나머지 조합은 "정직한 근접쌍 + 판별 질문"으로 자연스럽게 채워진다. 8개 전부 이 패턴으로 설계하니 왕복 계산 없이 한 번에 일관된 매트릭스가 나왔다.
**다시 마주칠 가능성**: 높음 — 이 저장소에 남은 8종(마들렌·티그레·버터바 등, idea-pastry.md MVP 밖) 확장 때도 매트릭스 작성이 반복될 것이므로, 이 설계 순서를 먼저 적용하면 저술 속도가 빨라진다.
