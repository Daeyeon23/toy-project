...
**상황**: Task 10에서 빵 이름 검색을 `.includes()`(부분 문자열)로 구현하고 spec.md Scenario 0-B의 성공 기준 "'치아' 입력 → '치아바타' 카드만 남고 무관한 빵은 안 보인다"를 테스트했더니, "포카치아"도 "치아"를 포함해 2개가 남아 테스트가 실패했다. spec은 4종(MVP) 시절 작성돼 25종 확장 후 실제로 이 예시가 거짓이 되는 걸 아무도 몰랐다.
**판단**: `.includes()`(부분 일치)를 `.startsWith()`(접두 일치)로 바꿔 spec의 리터럴 예시를 참으로 만들었다. 빵 이름 검색은 자동완성형 UX로 보는 게 자연스럽고, 이 저장소의 증상 검색(`filterSymptoms`, 동의어 사전 기반 부분 일치)과는 도메인이 달라 같은 매칭 전략을 강제할 이유가 없었다.
**다시 마주칠 가능성**: 중간 — spec의 구체적 예시(입력값→결과)는 작성 시점의 소규모 데이터셋에서만 검증된 것일 수 있다. 데이터가 나중에 대폭 늘어나는(4종→25종 같은) feature에서는, 구현 직전에 실 데이터로 spec의 예시 문구가 여전히 참인지 재확인하는 습관이 필요하다.
