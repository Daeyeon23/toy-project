...
- **수용 기준**:
  - [x] 증상 화면·결과 화면의 "다시 고르기" 버튼이 새 문구로 렌더되고, 클릭 시 기존과 동일하게 피커로 복귀하며 증상·결과가 초기화된다 (Scenario 0-C 동작 불변)
  - [x] 결과 화면의 면책 고지가 "일반적 베이킹 상식 기반의 가능성 있는 원인"으로 표시된다
  - [x] 피커 검색 placeholder와 빈 결과 안내가 "품목" 문구로 표시된다
  - [x] 기존 25종 전체 흐름(피커→진단→결과)이 문구 변경 후에도 동일하게 동작한다 (회귀)
  - [x] `bun run build` 성공
...
- **수용 기준**:
  - [x] 각 품목 대표 PEAKED 조합 → 기대 1순위 원인 재현
  - [x] "머핀" 검색 → "잉글리시 머핀"과 "머핀(퀵브레드)" 카드가 모두 나타나고 서로 다른 카테고리 그룹 아래 위치한다 (Scenario 0-E)
  - [x] "잉글리시 머핀" 선택 → 기존 이스트 반죽 증상 세트가 그대로 표시된다 (회귀 없음)
  - [x] "머핀(퀵브레드)" 선택 → 신규 화학팽창 머핀 증상 세트가 표시된다
  - [x] `qmuf-` 코드가 기존 `quick-` 코드와 문자열이 겹치지 않는다 (Task 12 레지스트리 대조 재확인)
...
- **수용 기준**:
  - [x] 기존 식빵·크루아상 E2E가 갱신된 면책 고지 문구("일반적 베이킹 상식 기반")로 통과한다
  - [x] `/` → 식빵 선택 → 증상 선택 → 진단 → 결과에 식빵 원인만 표시된다
  - [x] 결과 화면에서 "다시 고르기" → **마카롱**(제과류) 선택 → 이전 식빵 증상·결과가 남지 않고 마카롱 증상 세트로 전환된다 (Scenario 0-C, **빵→제과류로 도메인 경계를 넘는** 재선택)
  - [x] 마카롱으로 진단 → 결과에 마카롱 원인만 표시된다 (식빵 원인 미혼입, 품목 경계 격리가 도메인을 넘어도 유지됨)
  - [x] 전체 흐름 어디에도 원시 점수·퍼센트 숫자가 없다 (불변: 점수 비노출)
  - [x] 외부 호스트 네트워크 요청 0건 (불변: 클라이언트 완결)
- **검증**: `bun run test:e2e -- bread-doctor` · 증거 `artifacts/bread-doctor/evidence/task-16.png`

---

### Checkpoint: 최종 (v3)
- [x] 모든 테스트 통과: `bun run test` (296/296)
- [x] E2E 통과: `bun run test:e2e` (3/3)
- [x] 빌드 성공: `bun run build`
- [x] spec v3의 시나리오 0·0-A'·0-B·0-C·0-E 및 불변 규칙 전체(품목 경계 격리, 판별 질문 캡 예외 없음 포함)가 검증됨
